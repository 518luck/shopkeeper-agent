...
也负责按 id 把元数据查回业务实体供 Agent 节点使用
Repository 自身只关心“如何读写”，而“哪些写操作要放在同一笔事务里”，由 Service 层统一决定
...
class MetaMySQLRepository:
    ...
    async def get_column_info_by_id(self, id: str) -> ColumnInfo | None:
        ...
        if column_info_mysql is None:
            return None
        # Repository 对外返回业务实体，不把 ORM 模型泄露给 Agent 节点
        return ColumnInfoMapper.to_entity(column_info_mysql)

    async def get_table_info_by_id(self, id: str) -> TableInfo | None:
        """按表 id 查询表元数据，用于组装提示词里的表结构信息"""
        table_info_mysql: TableInfoMySQL | None = await self.session.get(
            TableInfoMySQL, id
        )
